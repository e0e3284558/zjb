    ...
    <div class="ibox-content margin-padding-0 relative-ibox-content">
        <form action="{{ url('users/departments/'.$department->id) }}" class="padding-20" method="post" id="dep-form">
            ...
            <div class="form-group">
                ...
                <div>
                    <input type="text" placeholder="部门名称" name="name" value="{{$department->name}}"
                           class="form-control">
                ...
                <div>
                    <select name="asset_category_id[]" id="" multiple class="form-control select2">
                        @foreach($asset_category as $v)
                            @if(in_array($v->id,$select_category))
                ...
                <div>
                    <select name="classify_id[]" id="" multiple class="form-control select2">
                        @foreach($classify as $v)
                            @if(in_array($v->id,$select_classify))
                                <option value="{{$v->id}}" selected>{{$v->name}}</option>
                            @else
                                <option value="{{$v->id}}">{{$v->name}}</option>
                            @endif
            ...
            <div class="form-group">
                ...
                <div>
                    <label class="radio-inline i-checks"> <input type="radio" name="status" class="icheck"
                                                                 value="1" {{ $department->status ? 'checked' : '' }}>
                        可用 </label>
                    <label class="radio-inline i-checks"> <input type="radio" class="icheck" name="status"
                                                                 value="0" {{ $department->status ? '' : 'checked' }}>
                        不可用 </label>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="">
                    <input type="number" placeholder="排序" name="sort" value="{{ $department->sort }}"
                           class="form-control">
                </div>
            </div>
            <div class="form-actions border-top ">
                ...
                <button type="submit" class="btn btn-success ladda-button" data-style="expand-left"><span
                            class="ladda-label">保存</span></button>
                <button type="button" class="btn btn-danger ladda-button" id="delete" data-style="expand-left">删除
                </button>
