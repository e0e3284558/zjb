...
@section("content")
    <div class="fh-breadcrumb full-height-layout-on white-bg layui-table-no-border">
        ...
        <table class="layui-table" lay-filter="data-user" lay-data="{id:'dataUser',height: 'full-194', url:'{{ url("asset_transfer") }}',page:true,limit:20,even:true,response:{countName: 'total'}}">
            <thead>
            <tr>
                <th lay-data="{fixed:'left',checkbox:true}"></th>
                <th lay-data="{field:'id', width:80, sort: true}">ID</th>
                <th lay-data="{field:'status', width:140, sort: true}">状态</th>
                <th lay-data="{field:'out_time', width:150, sort: true}">调出日期</th>
                <th lay-data="{field:'out_admin_id',templet: '#outAdminTpl', width:150, sort: true}">调出管理员</th>
                ...
                <th lay-data="{field:'put_area',templet: '#putAreaTpl', width:160, sort: true}">调入场地</th>
        ...
        <script type="text/html" id="outAdminTpl">
            @{{# if(d.out_admin){  }}
            @{{d.out_admin.name}}
            @{{# } }}
        </script>
        <script type="text/html" id="PputAdminTpl">
            @{{# if(d.put_admin){  }}
            @{{d.put_admin.name}}
            @{{# } }}
        </script>
        ...
        <script type="text/html" id="putAreaTpl">
            @{{# if(d.put_area){  }}
            @{{d.put_area.name}}
            @{{# } }}
        </script>
    ...
    <script type="text/javascript" >
        ...
        function edit() {
            if($("tbody input[type='checkbox']:checked").length >= 1){
                ...
                $.ajax({
                    url:'{{url('asset_transfer')}}/'+ids+"/edit",
