...
<div class="modal-body">
    <form id="signupForm2" class="form-horizontal " method="post" enctype="multipart/form-data" >
        ...
        <div class="row" >
            ...
            <table class="layui-table" id="borrow-add" lay-filter="data-borrow-add" lay-data="">
                <thead>
                <tr>
                </tr>
                </thead>
...
<script type="text/javascript">
    ...
    $( document ).ready( function () {
        ...
        layui.use('table', function () {
            ...
            //执行渲染
            table.render({
                elem: '#borrow-add' //指定原始表格元素选择器（推荐id选择器）
                , height: 410 //容器高度
                , url: '{{ url("borrow/slt_asset?type=2") }}'
                , cols: [[
...
//                    {title:'ID',field:'id', width:80, sort: true},
        ...
        $('#submitAssetsForm2').click(function () {
            ...
            var checkStatus = table.checkStatus('dataAssetAdd');
            var ids  = [];
            var tbody = $("#borrow_asset tbody");
            $.each(checkStatus.data,function(i,v){
                ...
                var str = "<tr>" +
                    ...
                    "<td>"+v.money+"</td>" +
                    "<td>"+v.area.name+"</td>" +
                    "<input type='hidden' name='borrow_asset_ids[]' value='"+v.id+"' >"+
                    "</tr>";
                if($('#borrow_asset input[value="'+v.id+'"]').length < 1){
                    tbody.append(str);
                }
            });
        });
