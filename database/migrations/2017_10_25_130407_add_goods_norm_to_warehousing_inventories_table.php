...
class AddGoodsNormToWarehousingInventoriesTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('warehousing_inventories', function (Blueprint $table) {
            ...
            $table->dropIndex(['foods_id']);//删除索引
            $table->string('foods_name')->nullable()->comment('物品名称')->change();
            $table->string('foods_num')->nullable()->comment('物品数量')->change();
            $table->string('foods_code')->nullable()->comment('物品编码')->change();
            $table->string('foods_barcode')->nullable()->comment('物品条码')->change();
